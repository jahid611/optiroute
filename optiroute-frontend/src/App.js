...
const COLORS = {
    DARK: '#3b4651', 
    BLUE: '#2b79c2', // C'EST CE BLEU LÀ QU'ON VEUT PARTOUT
    WHITE: '#ffffff',
    BORDER: '#dcdcde',
    GRAY_TEXT: '#6c757d',
    SUCCESS: '#28a745'
    ...
    const renderClientName = (name, slot) => {
        ...
        return (
            <div style={{display: 'flex', alignItems: 'center'}}>
                <img src={iconSrc} alt={slot} style={{width: '20px', height: '20px', marginRight: '12px'}} />
                <span style={{fontFamily: "'Oswald', sans-serif", fontSize: '1.1em', letterSpacing: '0.5px'}}>{name}</span>
            </div>
        );
    };
        ...
        <div style={rootContainerStyle(isMobileView)}>
                    ...
                    <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
                        {resetSuccess ? (
                            ...
                            <>
                                ...
                                <p style={{fontFamily: "'Inter', sans-serif", color: COLORS.DARK, marginBottom: '25px'}}>Voulez-vous vraiment effacer toute la tournée et réinitialiser les données ?</p>
                                <div style={{display: 'flex', gap: '10px'}}>
                                    ...
                                    <button onClick={confirmReset} disabled={resetLoading} style={{...gpsLinkStyle, justifyContent: 'center', backgroundColor: COLORS.DARK, color: '#fff', border: `1px solid ${COLORS.DARK}`}}>{resetLoading ? 'SUPPRESSION...' : 'OUI, EFFACER'}</button>
                                </div>
                            </>
                        )}
                    </div>
            ...
            <div style={panelContainerStyle(isMobileView)}>
                ...
                <div style={cardStyle}>
                    ...
                    <form onSubmit={handleAddMission}>
                        ...
                        <div style={{position: 'relative', marginBottom: '20px', userSelect: 'none'}}>
                            ...
                            >
                                <img 
                                    src={timeSlot === 'morning' ? '/icon-morning.svg' : '/icon-afternoon.svg'} 
                                    alt="icon" 
                                    style={{width: '20px', height: '20px', marginRight: '12px'}} 
                                />
                                <span style={{flex: 1}}>
                                    {timeSlot === 'morning' ? 'Matin (08h - 12h)' : 'Après-midi (14h - 18h)'}
                                </span>
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={COLORS.DARK} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{transform: isDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: '0.2s'}}>
                                    <path d="M6 9l6 6 6-6" />
                                </svg>
                            </div>
                            ...
                            {isDropdownOpen && (
                                <div style={{
                                    ...
                                    <div onClick={() => { setTimeSlot('morning'); setIsDropdownOpen(false); }} style={dropdownItemStyle}>
                                        <img src="/icon-morning.svg" alt="M" style={{width: '20px', marginRight: '10px'}} />
                                        Matin (08h - 12h)
                                    </div>
                                    <div style={{height: '1px', background: '#eee', margin: '0 10px'}}></div>
                                    <div onClick={() => { setTimeSlot('afternoon'); setIsDropdownOpen(false); }} style={dropdownItemStyle}>
                                        <img src="/icon-afternoon.svg" alt="A" style={{width: '20px', marginRight: '10px'}} />
                                        Après-midi (14h - 18h)
                                    </div>
                                </div>
                            )}
                        ...
                        <p style={{...helpTextStyle, marginBottom: '10px', textAlign: 'center'}}>
                            Ce point sera ajouté à la liste en attente.
                        </p>
                ...
                <div style={{...cardStyle, flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden'}}>
                    ...
                    <p style={{...helpTextStyle, marginBottom: '15px'}}>L'ordre optimal calculé par l'IA s'affichera ici.</p>
                                ...
                                <div key={index} style={missionItemStyle}>
                                    <div style={missionInfoStyle}>
                                        <div style={missionTitleStyle}>
                                            <span style={missionStepStyle}>{step.step}.</span> 
                                            {renderClientName(step.client, step.time_slot)}
                                        </div>
                                        <div style={missionAddressStyle}>{step.address.substring(0, 45)}...</div>
                                    </div>
...
const helpTextStyle = {
    color: COLORS.GRAY_TEXT, fontSize: '12px', fontFamily: "'Inter', sans-serif", fontStyle: 'italic', marginTop: '-5px', marginBottom: '15px', lineHeight: '1.4'
};
...
const resetButtonStyle = {
    ...
    // Ce filtre transforme le noir en Bleu DA #2b79c2
    filter: 'invert(38%) sepia(75%) saturate(1968%) hue-rotate(189deg) brightness(92%) contrast(89%)'
};
...
const missionStepStyle = { color: COLORS.BLUE, marginRight: '15px', fontWeight: '900', fontSize: '18px', fontFamily: "'Oswald', sans-serif" };
